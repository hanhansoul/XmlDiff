            ...
            for (int k = lower; k <= upper; k += 2) {
                ...
                if (k == ORIGIN - d || k != ORIGIN + d && lastD[k + 1] >= lastD[k - 1]) {
                    ...
                    if (generatingEditScript) {
                        ...
                        edit.operationMoveType = OperationMoveType.DELETE;
                    }
                } else {
                    row = lastD[k - 1];
                    if (generatingEditScript) {
                        edit.prev = editScript[k - 1];
                        edit.operationMoveType = OperationMoveType.INSERT;
                    }
                }
...
//        textOutputRight.append("<pre>");
        ...
        while (current != null) {
            ...
            for (int i = previousLeft + 1;
                 current.operationMoveType == OperationMoveType.DELETE && i < current.indexLeft ||
                         current.operationMoveType == OperationMoveType.INSERT && i <= current.indexLeft; i++) {
                if (i > 0) {
            ...
            for (int i = previousRight + 1;
                 current.operationMoveType == OperationMoveType.INSERT && i < current.indexRight ||
                         current.operationMoveType == OperationMoveType.DELETE && i <= current.indexRight; i++) {
                if (i > 0) {
            ...
            if (current.operationMoveType == OperationMoveType.INSERT) {
                ...
                while (current != null && current.operationMoveType == OperationMoveType.INSERT && current.indexLeft == b.indexLeft);
                ...
                do {
                    a = b;
                    b = b.next;
                } while (b != null && b.operationMoveType == OperationMoveType.DELETE && b.indexLeft == a.indexLeft + 1);
                boolean change = (b != null && b.operationMoveType == OperationMoveType.INSERT && b.indexLeft == a.indexLeft);
                if (change) {
                    if (current.indexLeft - 1 > 0) {
                        textOutputLeft.append(SPACE);
                    }
